...
; compares the \0-terminated strings given by ecx and edx, returning eax=1 if
; they are equal or eax=0 if not.
pm32_strcmp:
  push ecx
  push edx
.loop:
...
.parse_multiboot_header:
  ...
  ; header structure (module):
  ; dword [esi + 0] (common) - type (importantly, module = 3)
  ; dword [esi + 4] (common) - length of module (not including alignment)
  ; dword [esi + 8] - start paddr of module
  ; dword [esi + 12] - end paddr of module
  ; bytes [esi + 16] to [esi + length] - module "string"
  ...
  mov edx, [esi + 3 * 4]    ; figure out where our modules end so we can
  mov ecx, [modules_end] ; make sufficient pages for all of them
  call pm32_max
  mov [modules_end], edx

  lea edx, [esi + 4 * 4]          ; check path of loaded module against kernel
  mov ecx, kernel_path_search_str ; path to avoid jumping to a random module
  call pm32_strcmp
  cmp eax, 0
  je .not_kernel

  mov edi, [esi + 2 * 4]  ; module start, store so we can jump to it
  mov [kernel_paddr], edi ; once we enter 64-bit mode!

.not_kernel:
%ifdef DEBUG
  mov ecx, edi_mod_end_str
  call pm32_putstrln
  mov edx, [modules_end]
  call pm32_puthex
  mov dl, ' '
...
kernel_path_search_str:    ; path to boot to, if found as a module
  db "/boot/kernel.bin", 0
